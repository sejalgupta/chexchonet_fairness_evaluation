...
def split_train_eval_test(metadata_df_path, seed=42):
    ...
    df_train.to_csv(
        metadata_df_path.replace(".csv", "_train_" + str(seed)+ "_split.csv"), index=False
    )
    df_eval.to_csv(metadata_df_path.replace(".csv", "_eval_" + str(seed)+ "_split.csv"), index=False)
    df_test.to_csv(metadata_df_path.replace(".csv", "_test_" + str(seed)+ "_split.csv"), index=False)
